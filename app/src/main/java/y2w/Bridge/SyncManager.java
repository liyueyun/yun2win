...
public class SyncManager implements Serializable{
    ...
    private Map<Integer,SyncQueueEntity> syncQueues = new HashMap<Integer,SyncQueueEntity>();
    private List<SyncQueueEntity> entityList = new ArrayList<SyncQueueEntity>();
    ...
    public void addSync(SyncQueueEntity sq){
    ...
    private void syncQueueDeal() {
        try {
            if (entityList.size() == 0) {// 消息发送队列处理
                for(Map.Entry<Integer,SyncQueueEntity> entry : syncQueues.entrySet()){
                    SyncQueueEntity sq = entry.getValue();
    ...
    private void syncComplete(boolean isOk){
        if(isOk){
            if(entityList.size() > 0){
                SyncQueueEntity entity = syncQueues.get(entityList.get(0));
